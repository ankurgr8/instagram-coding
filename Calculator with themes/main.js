...
function getFormattedNumber(num) {
    ...
    let n = Number(num);
    let value = n.toLocaleString("en"); //return a string with representation of number
    return value;
}

function reverseNumberFormat(num) {
    return Number(num.replace(/,/g, '')); //Give a comma separated values
}
let operator = document.getElementsByClassName("operator");
for (let i = 0; i < operator.length; i++) {
    operator[i].addEventListener('click', function() {
        if (this.id == "clear") {
            //this id gives the current html element
            printHistory("");
            printOutput("");
